...
class ElphyFile(object):
    ...
    class Episode(object):
                ...
                if nremain > 0:
                    ...
                    for i, ech in enumerate(self.channels):
                        ...
                        data[i] = np.concatenate(
                            (ech.data, encoded_values * ech.dyu + ech.y0u))
        ...
        def _read_cyber_tags_data(self, file_handle):
            ...
            data['time'] = np.hstack((np.zeros(1, dtype='I'),
                                     np.repeat(times, 2),
                                     times[-1:]))
            data['value'] = np.vstack((np.zeros((2, 16), dtype='B'),
                                       np.repeat(bits, 2, axis=0)))
            return data
        ...
        def decode_episode_info(self, key):
            self.episode_info = _decode_user_info(self.file_info, key)
    ...
    class Channel(object):
        ...
        def _read_data(self, file_handle):
            ...
            if ep._format_option == 0:
                ...
                offset += sum([ch.data_size for ch in ep.channels[:self.channel_num]])
                file_handle.seek(offset)
                raw = file_handle.read(self.data_size)
...
def elphy_to_klusta(elphy_name, klusta_name=None, ep_nums='all',
                    chan_nums=range(64)):
    """
    ...
    - A text file (xxx_eplengths.txt) with each episode's number of samples
    """
    ...
    with open(klusta_name, 'wb') as bf:  # Initialize binary file
        with open(klusta_name.split('.')[0] + '_eplengths.txt',
            ...
            for iep in ep_nums:
                ...
                bf.write(ep_vecs.astype('<h').tobytes())  # Write data to binary file
                txtf.write(str(ch_lengths[chan_nums[0]]) + '\n')  # Write episode length to text file
...
if __name__ == "__main__":
    ...
    # filename = '../users/Margot/TEST_MARGOT_TUN_5EP_MISSING_DATA_CORRECTED.DAT'
    #
    # ef = ElphyFile(filename, read_data=False, read_spikes=False)
    # ep = ef.episodes[0]
    # ep.get_data(True)
    # ep.get_cyber_tags(True)
    #
    # c = ep.channels[0]
    #
    # s = ep.spikes
    ...
    #    elphy_to_klusta(filename)

    # maxNmov = 4000
    # key = {'Titre': ('str', 30), 'NbParam': 'int', 'actif': ('bool', 10),
    #        'NbTest': ('int', 10),
    ...
    # filename = 'E:/Users/Thomas/Thomas/PROJECTS/1311_AudioVis/data/behavior/150914 disks and sounds/151001/15-10-01_RECEDING-SOUNDS M4.DAT'
